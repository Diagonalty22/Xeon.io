...
module.exports = {
    ...
    operate: async (context) => {
        const { tdx, m, q, prefix, command, reaction, sleep, xreply } = context;
        
        if (!q) {
            await xreply(`Add input, Example: *${prefix + command} Hello World|en*`);
        ...
        try {
            ...
            await tdx.sendMessage(m.chat, { audio: fs.readFileSync(outputFile), ptt: true, mimetype: "audio/mpeg", fileName: "vn.mp3", waveform: [100, 0, 100, 0, 100, 0, 100] }, { quoted: m });
            await reaction(m.chat, "✅");
        } catch (error) {
