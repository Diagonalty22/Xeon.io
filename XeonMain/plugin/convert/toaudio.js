...
module.exports = {
    ...
    operate: async (context) => {
        const { tdx, m, prefix, command, qmsg, reaction, xreply, sleep, getRandom, mime } = context;

        if (!/video/.test(mime) && !/audio/.test(mime)) {
            await xreply(`Reply to a video message with the command ${prefix+command} to convert it to audio.`);
